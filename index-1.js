...
if (text.includes("_linkGroup")){
conn.groupInviteCode(id, GroupInviteCode, MessageType.text).then ((res) => {
conn.groupInviteCode (id)
	}) 
}
if (text.includes("_promote")){
	const teks = text.replace(/_promote /, "")
conn.groupMakeAdmin ("@g.us", ["@s.whatsapp.net", "${teks}"]).then ((res) => {
conn.sendMessage(id, 'Berhasil menjadikan admin', MessageType.text)
  }) 
}
if (text.includes("_demote")){
	const teks = text.replace(/_demote /, "")
conn.groupMakeAdmin ("@g.us", ["@s.whatsapp.net", "${teks}"]).then ((res) => {
conn.sendMessage(id, 'Berhasil menjadikan admin', MessageType.text)
  }) 
}
...
conn.sendMessage(id, o, MessageType.text);
  ...
  var buf = Buffer.from(response, 'base64'); 
           conn.sendMessage(id, buf, MessageType.image, {quoted: m,caption:'Nih bro'})
                      }
        )
        .catch(
            (error) => {
...
if (text.includes("_waifu"))
   {
	...
	var buf = Buffer.from(response, 'base64'); 
       ...
       conn.sendMessage(
            id,
              buf,MessageType.image, {quoted: m,caption:'>//<'})
...
if ( text == '_nulis') 
 ...
 if ( text == '_anime') 
  ...
  text1 = suara;
  var suara = text.replace(/_ttsid/g, text1);
...
if (text.includes("_anime"))
  ...
  if (text.includes("_hentong")){
    ...
    { let pa = `Nih Gan Hentong Nya Kalo Mau HD : ${res.data.url} `
    conn.sendMessage(id, pa,MessageType.text)
    })
 }
