...
/////////////////
const BotName = 'Stepen Bot Epek'; 
const kapanbotaktif = 'KETIKA HANYA DIAKTIFKAN'; 
const instagram = ' ';
const youtube = 'https://www.youtube.com/c/ExtOfficeEdmForLife';
...
conn.on('user-presence-update', json => console.log(json.id + ' presence is => ' + json.type)) || console.log('StepenEpek')
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : '' // participant exists when the message is from a group
   console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by ig:@extoffice_`)
})
...
if (text.includes("!cuaca")){
	const teks = text.replace(/!cuaca /, "")
	  axios.get(`https://mhankbarbar.herokuapp.com/api/cuaca?daerah=${teks}&apiKey=zFuV88pxcIiCWuYlwg57`).then ((res) =>{
		let anjay = `cuaca di daerah: ${teks} hari ini : angin : ${weather.result.angin} cuaca : ${weather.result.cuaca} deskripsi : ${weather.result.desk} kelembapan : ${weather.result.kelembapan} suhu : ${weather.result.suhu} udara : ${weather.result.udara}`;
  conn.sendMessage(id, anjay, MessageType.text);
})
}
...
if (text.includes("!img"))
  ...
  if (text.includes("!coderahasia"))
   {
    var items = ["sexy girl","cewek sexy","bigboobs cewek",];
    var nime = items[Math.floor(Math.random() * items.length)];
    var url = "https://api.fdci.se/rep.php?gambar=" + nime;
