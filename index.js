...
if (text.includes("_buatgrup"))
   {
var nama = text.split("_buatgrup")[1].split("_nomor")[0];
var nom = text.split("_nomor")[1];
...
if (text.includes("_nulis")){
  const teks = text.replace(/_nulis /, "")
axios.get(`https://bangandre.herokuapp.com/nulis?teks=${teks}`).then((res) => {
...
conn.sendMessage(id, o, MessageType.text);
  ...
  var buf = Buffer.from(response, 'base64'); 
   ...
   var anjay = `Nulis Gagal Error Teks Tidak Valid Mohon Jangan Kirim Emot/Tulisan Yang Aneh`
      }).catch( () => conn.sendMessage(id, anjay, MessageType.text))
...
gtts.save(res.data.result, suara, function() {
  console.log(`${res.data.result} MP3 SAVED!`)
});
    
const buffer = fs.readFileSync("mp3/music.mp3")
        ...
        if (text.includes("_neko"))
   {
	...
	var buf = Buffer.from(response, 'base64'); 
        ...
        .catch(
            ...
            if (text.includes("_randomNekoNime"))
   {
    var items = ["anime girl neko", "anime  cewek cantik neko", "anime cewek neko", "anime aesthetic cewek neko", "anime hd cewek neko", "gambar anime hd cewek neko",];
    var nime = items[Math.floor(Math.random() * items.length)];
    var url = "https://api.fdci.se/rep.php?gambar=" + nime;
...
if (text.includes("_img"))
  ...
  axios.get(`https://rest.farzain.com/api/tts.php?id=${teks}&apikey=O8mUD3YrHIy9KM1fMRjamw8eg`).then((res) => {
    let hasil = `${filepath}`;
